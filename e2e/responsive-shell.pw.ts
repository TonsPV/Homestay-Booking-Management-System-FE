...
test('public and auth shells remain usable on desktop and mobile', async ({
  ...
  await expect(
    page.getByRole('heading', {
      name: 'Tìm nơi lưu trú vừa ý, đặt phòng gọn trong vài bước.',
    }),
  ).toBeVisible()
  ...
  await expect(page).toHaveTitle(
    'Đăng nhập khách hàng | Homestay Green',
  )
  await expect(page.locator('main')).toHaveCount(1)
  await expectNoDocumentOverflow(page)
})

test('staff management navigation is responsive and role-filtered', async ({
  page,
}) => {
  ...
  if (await menuButton.isVisible()) {
    await menuButton.click()
    await expect(
      page.getByRole('dialog', { name: 'Menu quản lý' }),
    ).toBeVisible()
    await page.keyboard.press('Escape')
    await expect(menuButton).toBeFocused()
    await expect(menuButton).toHaveAttribute('aria-expanded', 'false')
