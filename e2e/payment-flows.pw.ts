...
test('customer VNPay initiation is idempotent and Return waits for Backend history', async ({
  page,
}) => {
...
test('admin sees stale refunds, reconciles pending VNPay, and refunds manual payment once', async ({
  ...
  await expect(
    page.getByText(/1 giao dịch cần được kiểm tra/i),
  ).toBeVisible()
  ...
  await page
    ...
    .dblclick()
  await expect(
    page.getByText('Đối soát hoàn tiền thành công'),
  ).toBeVisible()
  expect(reconcileCount).toBe(1)

  await page
    .getByRole('button', { name: 'Hoàn tiền giao dịch #93' })
    .click()
  await page.getByLabel('Lý do hoàn tiền').fill('Khách đổi kế hoạch')
  await page
    .getByRole('button', { name: 'Xác nhận hoàn tiền' })
    .dblclick()
...
test('staff can review payments but cannot refund or reconcile', async ({
  page,
}) => {
...
test('admin refund pending and reconcile refresh payment and cached dashboard state', async ({
  ...
  await page
    .getByRole('link', { name: 'Quản lý thanh toán' })
    .click()
  await page
    .getByRole('button', { name: 'Hoàn tiền giao dịch #94' })
    .click()
  await page.getByLabel('Lý do hoàn tiền').fill('Khách đổi kế hoạch')
  await page
    .getByRole('button', { name: 'Xác nhận hoàn tiền' })
