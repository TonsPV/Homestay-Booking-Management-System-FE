...
test("admin login continues through amenity, room type, room, image cover and calendar block", async ({
  ...
  await page.route("**/api/v1/**", async (route) => {
    ...
    /* The unified login flow probes the customer endpoint first; keep the
     * 401 local so no request reaches the real backend. */
     ...
     * list so the fake token does not get the session revoked. */
    if (path.endsWith("/management/bookings") && method === "GET") {
      await route.fulfill({
        body: JSON.stringify({
          ...envelope([], path),
          meta: {
    ...
    if (path.endsWith("/management/rooms/31/blocks") && method === "POST") {
      ...
      blockedDates = ["2099-05-10", "2099-05-11"].map((stayDate, index) => ({
        booking: null,
        id: String(801 + index),
        reason: "Bảo trì định kỳ",
        status: "BLOCKED",
  ...
  await page.getByRole("button", { name: "Quay lại chi tiết phòng" }).click();
  await expect(page).toHaveURL(/\/management\/rooms\/31$/);
