...
test('login 429 preserves the form and exposes the Retry-After cooldown', async ({
  ...
  await expect(
    page.getByText(/Bạn đã thao tác quá nhiều lần/),
  ).toBeVisible()
  await expect(
...
test('a Backend 403 remains an explicit permission state without clearing session', async ({
  ...
  await expect(
    page.getByText(
      'Bạn không có quyền xem dữ liệu thanh toán. Mã tra cứu: req-payment-forbidden.',
    ),
  ).toBeVisible()
  await expect(page).toHaveURL(/\/management\/payments$/)
...
test('a Backend 409 keeps its request ID in an actionable operation error', async ({
  page,
}) => {
  ...
  await expect(
    page.getByText(
      'Tiện nghi vẫn đang được sử dụng. Mã tra cứu: req-amenity-conflict.',
    ),
  ).toBeVisible()
})
