...
test('live counter and customer booking journeys use Backend state', async ({
  ...
  await page.getByLabel('Tên liên hệ').fill('Live Counter Guest')
  await page.getByLabel('Số điện thoại').fill(counterPhone)
  await page.getByRole('button', { name: 'Tạo booking' }).click()
  await expect(
    page.locator('span').filter({ hasText: /^Chờ thanh toán$/ }).first(),
  ).toBeVisible()
  await expect(
    page.locator('span').filter({ hasText: /^Chưa thanh toán$/ }).first(),
  ).toBeVisible()
  ...
  await page.getByLabel('Số khách').fill('2')
  await page.getByLabel('Tên liên hệ').fill('Live Payment Guest')
  ...
  await expect(
    page.locator('span').filter({ hasText: /^Đã thanh toán$/ }).first(),
  ).toBeVisible()
  ...
  await paymentRow
    .getByRole('button', { name: /Hoàn tiền giao dịch/ })
    .click()
  await page
  ...
  await expect(
    page.locator('span').filter({ hasText: /^Đã hoàn tiền$/ }).first(),
  ).toBeVisible()
  await expect(
    page.locator('span').filter({ hasText: /^Đã hủy$/ }).first(),
  ).toBeVisible()
