...
describe('RoomImageManager', () => {
  it('previews and submits a selected image, then clears the form', async () => {
    ...
    render(<RoomImageManager onClose={vi.fn()} roomId={room.id} />)
  ...
  it('renders a normalized server upload error', () => {
    mocks.createError = new Error('Ảnh không hợp lệ hoặc đã bị từ chối.')

    render(<RoomImageManager onClose={vi.fn()} roomId={room.id} />)

    expect(
      screen.getByRole('alert'),
    ).toHaveTextContent('Ảnh không hợp lệ hoặc đã bị từ chối.')
  })
