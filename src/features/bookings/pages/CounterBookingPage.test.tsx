...
import { CounterBookingPage } from "./CounterBookingPage";
...
describe("CounterBookingPage", () => {
  it("clears the selected room when the dates change", async () => {
    ...
    expect(
      screen.getByRole("region", { name: /Tóm tắt booking/i }),
    ).toBeInTheDocument();
    ...
    await waitFor(() =>
      expect(
        screen.getByRole("region", { name: /Tóm tắt booking/i }),
      ).toHaveTextContent(/Chưa chọn phòng/i),
    );
    expect(screen.queryByLabelText(/Họ tên khách/i)).not.toBeInTheDocument();
  });
  ...
  it("shows the selected room, stay and estimated base price in the summary", async () => {
    mockAvailability();
    renderPage();
    fillStayCriteria();
    ...
    expect(summary).toHaveTextContent(/Theo giá cơ bản × số đêm/i);
    expect(summary).not.toHaveTextContent("Nguyễn Văn B");
    expect(summary).not.toHaveTextContent("0901234567");
    expect(screen.getByLabelText(/Họ tên khách/i)).toHaveValue("Nguyễn Văn B");
  });

  it("keeps an actionable empty summary and hides customer fields until room selection", () => {
    mockAvailability();
    renderPage();
