...
describe('CounterRoomPicker', () => {
  it('shows idle guidance when criteria are missing', () => {
    ...
    expect(
      screen.getByText(/Nhập ngày nhận, ngày trả và số khách/i),
    ).toBeInTheDocument()
  })
  ...
  it('marks the selected room as checked', () => {
    ...
    expect(
      screen.getByRole('radio', { name: /Phòng 101/i }),
  ...
  it('renders pagination controls when multiple pages exist', async () => {
    ...
    const next = await screen.findByRole('button', { name: /Trang sau/i })
