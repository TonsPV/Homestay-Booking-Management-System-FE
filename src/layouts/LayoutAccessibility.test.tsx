...
describe('layout accessibility contracts', () => {
  ...
  it('traps focus in the staff drawer, filters admin routes, and restores focus', async () => {
    const user = userEvent.setup()
    renderManagementLayout()
    const trigger = screen.getByRole('button', {
      name: 'Mở menu quản lý',
    })

    await user.click(trigger)

    const drawer = screen.getByRole('dialog', { name: 'Menu quản lý' })
    const drawerQueries = within(drawer)
    const closeButton = drawerQueries.getByRole('button', {
      name: 'Đóng menu quản lý',
    })

    await waitFor(() => expect(closeButton).toHaveFocus())
    expect(document.body.style.overflow).toBe('hidden')
    expect(
      drawerQueries.queryByRole('link', { name: 'Nhân viên' }),
    ).not.toBeInTheDocument()
    expect(
      drawerQueries.queryByRole('link', { name: 'Khách hàng' }),
    ).not.toBeInTheDocument()
