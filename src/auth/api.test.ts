...
describe("auth API contract", () => {
  ...
  it("sends the Google credential and optional phone to the customer route", async () => {
    const customerLogin: LoginResponse = {
      accessToken: "customer-google-token",
      actorType: "customer",
      customer,
      expiresIn: 3600,
      tokenType: "Bearer",
    ...
    const fetchMock = vi
      .fn()
      .mockResolvedValue(successfulEnvelope(customerLogin));
    vi.stubGlobal("fetch", fetchMock);
    ...
    expect(requestFrom(fetchMock)).toMatchObject({
      body: { credential: "google-id-token", phone: customer.phone },
      method: "POST",
      url: "http://localhost:3000/api/v1/auth/customers/google/login",
    });
